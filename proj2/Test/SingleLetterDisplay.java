...
public class SingleLetterDisplay extends Application {
    ...
    private class KeyEventHandler implements EventHandler<KeyEvent> {
        ...
        /* see if keeping current x position can help */
        double currentX = 5.0;
        double currentY = 5.0;
        ...
        /* try to have a list of each letter entered */
        ArrayList<Text> enteredText = new ArrayList<>();
        ...
        public void handle(KeyEvent keyEvent) {
            if (keyEvent.getEventType() == KeyEvent.KEY_TYPED) {
                ...
                if (characterTyped.length() > 0 && characterTyped.charAt(0) != 8) {
                    ...
                    displayText = new Text(characterTyped);
                    displayText.setTextOrigin(VPos.TOP);
                    displayText.setFont(Font.font(fontName, fontSize));

                    currentX = enteredText.size() > 0 ? Math.round(enteredText.get(enteredText.size() - 1).getLayoutBounds().getWidth()) + 1 + currentX
                                    : 5;

                    if (currentX > 495) {
                        currentY += new Text("a").getLayoutBounds().getHeight();
                        currentX = 5;
                    }

                    // System.out.println(currentX);
                    displayText.setX(currentX);
                    displayText.setY(currentY);
                    enteredText.add(displayText);
                    root.getChildren().add(displayText);
